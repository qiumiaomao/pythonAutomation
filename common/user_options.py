# user_options.py模块用于定义并接收用户输入
# 用户输入扩展类测试框架的功能，通过用户输入，测试框架查找用户指定文件（夹）、筛选用户指定的测试用例以及运行在用户指定的测试环境等自定义需求
...
def parse_options(user_options=None):
    ...
    parser.add_argument('-env', action='store', default='qa', dest='default_env', metavar='target environment',
                       choices=['qa', 'prod'], help='Spectfy test environment')

    parser.add_argument('-t', action='store', default='.'+ os.sep +'tests', dest='test_targets', metavar='target run path/file',
                       help='Spectfy run path/file')

    parser.add_argument('-i', action='store', default=None, dest='include_tags_any_match',
                       metavar='user provided tags, string only, separate by comma without an spacing among all tags. if any user provided tags are defined in test class, the test class will be considered to run.',
                       help='Select test cases to run by tags, separated by comma, no blank space among tag values.')

    # 添加并发数目，默认为可用的cpu个数
    parser.add_argument("-n", action="store", type=int, default=cpu_count(), dest="test_thread_number", metavar="int number", help="Specify the number of testing thread run in parallel, default are the cpu number.")
    ...
    if options.test_targets:
        if not os.path.isdir(options.test_targets) and not os.path.isfile(options.test_targets):
            parser.error("Test targets must either be a folder path or a file path, it either be absolute path or relative path, if it isrelative, it must relative to tests folder under your root folder")
    ...
    if options.default_env:
        try:
            module_package = '.' + os.sep + 'config' + os.sep + 'test_env'
